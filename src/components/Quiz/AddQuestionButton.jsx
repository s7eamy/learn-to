...
const AddQuestionButton = ({
  ...
  const handleAnswerChange = (idx, value) =>
    setAnswers((prev) =>
      prev.map((a, i) => (i === idx ? { ...a, text: value } : a)),
    );
  const handleToggleCorrect = (idx) =>
    setAnswers((prev) =>
      prev.map((a, i) => (i === idx ? { ...a, isCorrect: !a.isCorrect } : a)),
    );
  ...
  const confirmDelete = () => {
    if (editQuestion && editQuestion.id) {
      ...
      if (onQuestionDeleted) {
        ...
        fetch(`/api/quizzes/${quizId}/questions/${editQuestion.id}`, {
          method: "DELETE",
        })
          .then((res) => {
          ...
          {answers.map((ans, idx) => (
            ...
            >
              ...
              <FormControlLabel
                ...
                sx={{ ml: 1 }}
              />
              <IconButton onClick={() => handleRemoveAnswer(idx)}>
                <DeleteIcon />
              </IconButton>
            </Box>
          ))}
          <Button onClick={handleAddAnswer} sx={{ mt: 1 }}>
            + Add Answer
          </Button>
        </DialogContent>
        <DialogActions>
