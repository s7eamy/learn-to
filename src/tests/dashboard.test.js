...
global.fetch = jest.fn((url) => {
	if (url === "/api/sets") {
		...
		//tam vienam autentifikavimo testui
		return Promise.resolve({
			ok: true,
...
describe("Dashboard", () => {
	...
	test("displays logged in username after successful fetch", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText(/Logged in as: TestUser/i)
			).toBeInTheDocument();
		});
	});

	// Test that when fetch fails, the Dashboard displays "Guest" as the user.
	test.skip("displays Guest when fetch fails", async () => {
	...
	test("closes the set creator dialog when clicking the close icon", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByPlaceholderText(/Name your set/i)
			).toBeInTheDocument();
		});
		...
		await waitFor(() => {
			expect(
				screen.queryByPlaceholderText(/Name your set/i)
	...
	test("allows to press create button after prerequisites are met", async () => {
		...
		const dashboardCreateButton = screen.getByRole("button", {
			name: /create/i,
		});
		...
		const createButtonInDialog = within(dialog).getByRole("button", {
			name: /create/i,
		});
